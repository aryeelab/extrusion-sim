...
df %>% filter(effective_dist<=30) %>% 
  group_by(sim, cond, effective_dist) %>% summarize(n=n()) %>% group_by(sim, cond) %>% mutate(cumsum = cumsum(n)) %>% ggplot(aes(effective_dist, cumsum, color=cond)) + geom_line() + xlim(0,30) + ylim(0, 60000) + theme_bw() + ggtitle(rds) +
  facet_wrap(~sim)
