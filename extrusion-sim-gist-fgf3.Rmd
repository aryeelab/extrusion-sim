...
```{r}
ctcf_occ_rate <- 1
sim <- Sim(8, polymer_length_bp = 240e3*8, processivity_bp = 150e3, bead_size = 1000, ctcf_capture_rate=0.75)
...
```{r}
df %>% ggplot(aes(effective_dist)) + geom_histogram() + facet_wrap(~cond) + theme_bw() + ylim(0, 2000)
```

```{r}
df %>% ggplot(aes(cond, effective_dist)) + geom_boxplot() + theme_bw()
